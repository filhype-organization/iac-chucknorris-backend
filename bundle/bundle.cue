// bundle.cue — Timoni Bundle pour le déploiement GitOps chuck-norris-backend.
//
// Ce fichier est la source de vérité pour le déploiement du backend en production.
// Le job CI update-iac met à jour le tag image automatiquement.
// Le CronJob timoni-runner (namespace timoni-system) applique ce bundle toutes les 2 minutes.
bundle: {
	apiVersion: "v1alpha1"
	name:       "chuck-norris-backend"
	instances: {
		"chuck-norris-backend": {
			module: {
				url:     "oci://registry-1.docker.io/leeson77/chuck-norris-backend-timoni"
				version: "latest"
			}
			namespace: "chuck-norris"
			values: {
				image: {
					tag: "1f41950" // CI:backend updates this line automatically — do not edit manually.
				}
				sealedSecrets: {
					auth0: {
						encryptedClientSecret: ""
					}
					database: {
						encryptedUsername: ""
						encryptedPassword: ""
					}
				}
			}
		}
	}
}
